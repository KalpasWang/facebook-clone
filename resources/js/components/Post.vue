  ...
  <div class="bg-white rounded shadow w-full max-w-md mt-6">
    ...
    <div class="flex justify-between pt-1 m-2" style="border-top: 1px solid #ccc">
      <button class="flex justify-center py-1 rounded-lg text-sm text-gray-700 w-full hover:bg-gray-200 focus:outline-none" @click="clickLikeBtn">
        <i v-if="isUserLikes" class="fas fa-thumbs-up text-blue-500 w-5 h-5 text-xl"></i>
        ...
        <p :class="['ml-2', { 'text-bkue-500': isUserLikes }]">讚</p>
      </button>
...
export default {
  ...
  computed: {
    ...mapGetters(['authUser']),
    isUserLikes() {
      return this.post.data.attributes.likes.data.some(item => item.data.user_id === this.authUser.data.user_id);
    }, 
  },
  methods: {
    clickLikeBtn() {
