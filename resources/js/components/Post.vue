...
export default {
  ...
  computed: {
    ...mapGetters(['authUser']),
    isUserLikes() {
      this.post.data.attributes.likes.data.some(item => {
        if(item.data.user_id === this.authUser.data.user_id)
          return true;
      });
    }, 
  },
